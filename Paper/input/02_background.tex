...
different overview as shown. % here. %Figure \ref{wienbot:webbot}
%
%\begin{figure}[h!]
%	\label{wienbot:webbot}
%	\caption[Vienna City Website vs. WienBot]{Screenshots of the Vienna City Website vs. Wienbot}
...
\begin{figure}[H]
	...
	\label{vca:start}
	\includegraphics[width=0.85\textwidth]{livebot2}
\end{figure}

\subsection*{The Virtual Citizen Assistant and LeiKa}

The Virtual Citizen Assistant starts with a greeting and an input field as seen in Figure \ref{vca:start}. Based on the question text entered by the user, a query goes to the Solr back-end and the responses from there are processed to deliver available options, which the user can choose from.
Once a question is answered, the chatbot continues to ask if the user is interested in knowing more about topics related to the service, such as cost or processing time.
The chatbot is a rich implementation with many features, e.g. the ability to decipher locations based from input postal codes (Postleizahl, PLZ) among many others. We use the features we extract as an orientation on how our voice assistant should be implemented and what inputs and outputs it needs to support. As we anticipate a voice-first approach, we also demonstrate how these should or should not be adapted in Section \ref{designalexa:bestprac}.
...
By deploying an instance of the chatbot locally, we understand that it is built using Java for the Solr server implementation and Groovy, a Java-syntax-compatible OOP language, to operate a web app on top of Solr to handle user inputs and display answers formatted inside a web browser (e.g. links for navigation in the conversation and standard text). We also understand that the implementation is designed to include other services than the Berlin scenario. At the time of this research, another chatbot was in the making for the City of Hamburg. Both chatbots were able to connect with a third service that can serve in other domains if the user has questions beyond the scope of public services. A nutrition bot was taken as an example.
In the service domain related to Berlin, we compile a few files relevant to our use-case, all of which are in JSON format. Java and Groovy code were not considered since we anticipate a different platform. Since JSON is an industry standard, we look for possibilities to parse this information into a system that can read them in and be structured to understand the file format. 
%For their flexible 
%structure relevant to our work.

Starting with \mintinline{bash}{./ssds-data-berlin/json/dienstleistungen/} \mintinline{bash}{dienstleistung.json} (the file has been restructured since this version), we analyse the content of the nodes (Figure \ref{LeiKaJSON1}):
...
\begin{figure}[H]
	\caption[File Structure of \lstinline|Dienstleistungen.json|]{Primary Nodes of \mintinline{java}{Dienstleistungen.json} including metadata about each service, e.g. processing time and cost.}
	\label{LeiKaJSON1}
	\includegraphics[width=0.9\textwidth]{DLprim}
\end{figure}

\begin{figure}[t]
	\caption{Secondary Nodes of \mintinline{java}{Dienstleistungen.json} show relevant follow-up nodes to the primary service nodes}
	\label{LeiKaJSON2}
	\includegraphics[width=\textwidth]{DLsec}
\end{figure}

There are other files related to the districts locations (\mintinline{bash}{./ssds-data-berlin/json/districts.json}, the authorities' offices and their opening hours (\mintinline{bash}{./ssds-data-berlin/json/behoerden/behoerden.json} which will help us indirectly in our implementation later on. The web app revolves around the following algorithm: once a public service has been caught from the user's input, the chatbot shows the description of the service, followed by a series of questions beginning with a yes/no one to determine if the user is interested in knowing more about the cost of the service, followed by another to pompt if the chatbot should show information of other metadata of the service, e.g. processing time or which authorities are responsible to offering the service. The user navigates by clicking on the links provided inside the chatbot conversation. 
...
The content of these nodes is provided through the federal IT Service Centre Berlin (IT Dienstleistungszentrum) and is part of the %Federal Catalogue of Public Services (
LeiKa%) 
\footnote{\url{https://de.wikipedia.org/wiki/LeiKa}}. In a similar implementation for the Facebook chatbot Berlina (currently in Beta), Hassmann and Müller \cite{hassmannMlr:berlina} explore a similar implementation and conclude with the use of JSONs being a flexible foundation for development.
...
\begin{itemize}
	\item \mintinline{bash}{d115}: An index core designed to query the latest public services available on Berlin.de. \\ Example services: ``Personalausweis beantragen - issuing a passport'', ``Blindenhilfe - monetary relief for the blind''.\\  This core initally included 616 nodes at the beginning of this research and now varies between 670 and 680 services depending on the original LeiKa. We will use this core later on as our primary API.
	
	\item \mintinline{bash}{d115Extern}: An index core designed to include external Hyperlinks outside the service.berlin.de subdomain.\\
	Examples: ``Gentechnik - gene technology'' refers to 
	\url{https://www.berlin.de/lageso/gesundheit/gesundheitsschutz/gentechnik/}
	
	\item \mintinline{bash}{d115Locations}: An index core designed to query locations of authority offices offering a public service (from the \lstinline|d115| core)``Kfz-Zulassungsbehörde-Lichtenberg"''
	
	\item \mintinline{bash}{d115Spelling}: An index core designed to make the chatbot able to determine spelling mistakes in names of entities (e.g. offices, services) and automatically correct these in the conversation flow
	
	\item \mintinline{bash}{d115Topics}: An index core designed to query the database for topics. Each topic collects a set of services that are of the same interest group. For instance, a topic ``Ausländerangelegenheiten'' comprises all sorts of visa and residence permit types. Another topic on ``Auto und Verkehr'' includes services like parking options, paying fines and issuing a driver's licence.
	%trelated .
	
	\item \mintinline{bash}{smalltalk}: An index core designed to make the chatbot more interactive by including simple questions and answers to them not related to Berlin.de's service catalogue

	
	
\end{itemize}

Noteworthy is that in the German public administration system, most public services are carried out through each of the 16 States with very few services offered on a national level. LeiKa is a way to standardise how public service organs of each State fit into the same legal framework for federal regulations. For instance, it is a requirement for each resident in Germany to register themselves at a local authority. How this is carried out is left to each state to determine. In many states a foreigner can register themselves at the aliens office. In Berlin, foreigners are required to register only at a Bürgeramt. In order to make sure that the legal framework is covered through the services in every state, LeiKa serves as a catalogue to keep track of how and through which organs the law is enforced.


We cover here an excerpt of the node elements we will later use %more or less in the same structure of the JSON file containing the interaction model:\\
in a similar structure for our own front- and back-end. 
As it is important to have the structure of these files for our implementation, the next table contains a compact form of each core.
...
\end{table}
		...
		Tables \ref{dienstleistung:summary1} and \ref{dienstleistung:summary2} are available on \mintinline{bash}{/home/ssds/ssds-solr/data/cores/}

\begin{table}[htbp]
	\caption[Structure of Solr Cores (cont'd)]{Structure of Solr cores 3-6 (cont'd)}
	\label{dienstleistung:summary2}
	\begin{tabu} to \textwidth{|X[2]|X|X[7]|}



\lstinline|d115Extern| & 1488 & 
\begin{tabu} to 5cm {lll}
	\lstinline|id| &  \lstinline|d115Topic| & \lstinline|d115Name| \\  \lstinline|d115Link| & \lstinline|d115Keywords| & \lstinline|_version_|\\ 
\end{tabu}
\\ \hline


\lstinline|d115Extern| & 1232 & 
\begin{tabu} to 5cm {lll}
	\lstinline|id| &  \lstinline|d115Topic| & \lstinline|d115Name| \\  \lstinline|d115Link| & \lstinline|d115Keywords| & \lstinline|_version_|\\ 
\end{tabu}
\\ \hline
...
\begin{table}[htbp]
	\caption[Relevant Nodes in API Query Results]{Relevant Nodes in Solr API Query Results \mintinline{java}{Dienstleistungen.json} in D115 Core }
	...
	\begin{tabu} to \textwidth{|X|X|X[2]|}
		...
		\lstinline|<string>| & \shortstack[l]{https://service.berlin\\.de/dienstleistung/\\326233/}
		& link to public service URL % on  \href{https://service.berlin.de}{service.berlin.de} 
		\\
		
		\lstinline|d115Name| \lstinline|<string>|  &  Fiktions-bescheinigung % Fiktionsbescheinigung 
		& public service name as listed on Berlin.de %\href{https://service.berlin.de}{service.berlin.de/dienstleistungen} 
		\\		
		
		\lstinline|ssdsAll <string>| & ... & \shortstack[l]{additional captive search terms,\\ synonyms used by Virtueller \\ Bürgerassistent. Found to be \\ helpful in conversations \\ with the chatbot} \\		
		
		\shortstack[l]{\lstinline|d115Description| \\ \lstinline|<string>|}  &... & \shortstack[l]{Introductory paragraph from \\the service web page \\ 
		Includes HTML tags}
...
\todo{Amazon German Skills have become available since feb 2017!}

\begin{figure}[h]
	\caption[Alexa Skills by Volume (2016)]{Alexa Skills by Volume (2016) \cite{voicelabs:trends}}
	\label{skillsbyvol}
