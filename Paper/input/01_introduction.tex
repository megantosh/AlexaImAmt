...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
With the aforementioned, we try to think how voice assistants could become useful and why we want to invest in such technology, juxtaposing it to available alternatives.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Since the point of availing voice assistance is to act in a person's interest, we also want an information system not to confuse us or to limit our cognitive abilities.
...
For those who have minimal understanding of the language, there are possibly also options to provide help in the native language if the user is given the options he/she can answer a prompt with. At the very least, a VUI could still give feedback to the user of whether it understands the language or not, since algorithms for detecting a language are not as complex as answering the question once the language and its dialect (also known as locale) is deciphered. %understood.
All of which are optimal use cases for ML approaches and algorithms such as term weighting 
%discussed in Section \ref{discussions} 
or based on simpler approaches to localization from an original text, such the corpus-based mapping approach. WordNet is one such example. %Auch: Stammsprache,...refer to IRS lecture notes
...
\begin{itemize}
	...
	\item We want our solution to be voice-first.
...
\begin{itemize}
	...
	\item The endpoint used is the same of the service catalogue of Berlin.de's %\textsc{
	Virtual Citizen Assistant
	%	}
	.
	
	\item Synonyms should be supported
	
	
	\item The software should use an underlying ML-based intelligence.
	
	\item content management should be minimal 
	
\end{itemize}


Further requirements with respect to the programming platform are discussed in Chapter~\ref{mainone}.
...
%} 
\ref{stateofzart} 
on the available platforms %in Section \ref{devmodels} 
and explain %in the subsequent Section \ref{choiceOfPlatform} 
why we choose Amazon, going over the idea behind Alexa's Skills
...
explains our implementation solution. We elaborate in particular on how we deal with individual public services in the interaction model from front-end and back-end perspective, how we connect the web services together and analytically document the code structure and endpoints, in addition to the deployment process.
...
use cases. We base our evaluation models on a survey we conduct that helps us specify requirement details, as well as automated tests provided by the framework we use in addition to usability, performance and unit tests. 
